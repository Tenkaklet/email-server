...
const handleEmails = (err, req,res,next) => {
  if (err instanceof FooError || err instanceof BarError) {
    console.log(`Sending email to ${ALERT_FROM_EMAIL}`);
    const email = {
      from: ALERT_FROM_EMAIL,
      to: ALERT_TO_EMAIL,
      subject: `This went wrong ${err.name}`,
      text: `${err.name}`,
      html: `<h1>This went wrong</h1><br /><p>${err.stack}</p>`
    }
    sendEmail(email);
  }
  next();
